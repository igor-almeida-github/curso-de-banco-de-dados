# Agrupamento
CREATE DATABASE k_agrupamento_59;

USE k_agrupamento_59;
...
SELECT produtos.nome, fabricantes.nome AS fabricante, produtos.quantidade, tipos.nome AS tipo
    FROM produtos, fabricantes, tipos
    WHERE produtos.id_fabricante = fabricantes.id AND produtos.id_tipo = tipos.id;

# Funções de agrupamento e ordenação

-- Group By
SELECT t.nome, f.nome AS Fabricante, SUM(p.quantidade) AS 'Quantidade em Estoque'
